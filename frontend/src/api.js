...
export async function listChatbots(filter = "") {
  ...
  const out = (listOfAssistant || []).map(b => {
    // be robust about fields returned by backend
    const key = (b && (b._id || b.id || b.key || "")).toString();
    const displayName = b?.displayName || b?.name || b?.label || "";
    return {
      key,
      name: (labels[key] && String(labels[key]).trim()) || displayName || key,
      defaultName: displayName || key,
    };
  });
